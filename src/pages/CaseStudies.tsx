...
function CSHero() {
  return (
    <section className="relative bg-ink text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-container mx-auto px-10 pt-[80px] pb-[80px]">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="mt-16 pt-6 border-t border-white/10 grid grid-cols-4 gap-10"
        >
          {[['12+','Enterprise engagements'],['500+','Professionals trained'],['6','Expertise areas'],['98%','Satisfaction rate']].map(([k,v]) => (
            <motion.div key={v} variants={fadeUp}>
              <div className="font-display text-[48px] font-semibold tracking-[-0.02em] text-white">
                {k.replace(/\d+/, n => n)}<span className="text-brand">{k.replace(/\d/g,'')}</span>
              </div>
              <div className="mt-2 text-[13px] text-white/50">{v}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
function CSFeatured() {
  return (
    <section className="px-10 py-24 bg-paper">
      <div className="max-w-container mx-auto grid gap-20">
          ...
          return (
            ...
            >
              ...
              <div className="relative overflow-hidden" style={{ aspectRatio: '21/7' }}>
                <img
                  src={i === 0 ? IMG.case1 : IMG.case2}
                  alt={`${c.client} engagement`}
                ...
                <div className="absolute inset-0 flex items-end p-10">
                  <div>
                    ...
                    <h2
                      className="mt-2 font-semibold tracking-[-0.02em] leading-[1.1] text-white"
                      style={{ fontSize: 'clamp(28px, 3vw, 42px)' }}
                    >
                      {c.title}
                    </h2>
                    <div className="mt-2 font-display text-[20px] font-semibold text-white/80">{c.client}</div>
                  </div>
                </div>
              </div>

              {/* Details grid */}
              <div className="grid grid-cols-[180px_1fr] gap-14 p-10 bg-paper">
                <div>
                  ...
                  <div className="font-mono text-[11px] tracking-[0.12em] uppercase text-muted mt-5 mb-1">Duration</div>
                  <div className="text-[14px]">{d.duration}</div>
                  <div className="font-mono text-[11px] tracking-[0.12em] uppercase text-muted mt-5 mb-2">Scope</div>
                  <ul className="list-none p-0 grid gap-1 text-[13px] text-ink-3">
                    {d.scope.map(s => <li key={s}>· {s}</li>)}
                  </ul>
                </div>
                <div>
                  <p className="text-[16px] leading-[1.7] text-ink-3">{d.long}</p>
                  <div className="mt-8 grid grid-cols-3 gap-8 pt-6 border-t border-line">
                    {d.outcomes.map(([k, v]) => (
                      <div key={v}>
                        <div className="font-display text-[40px] font-semibold text-brand-dark tracking-[-0.02em] leading-none">{k}</div>
                        <div className="text-[13px] text-muted mt-2">{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
    ...
    <section className="relative px-10 py-24 bg-ink overflow-hidden">
      ...
      <div className="relative z-10 max-w-container mx-auto">
        <motion.div
          ...
          whileInView="show"
          viewport={viewport}
          className="grid grid-cols-[180px_1fr] gap-16 mb-12"
        >
          <motion.div variants={slideLeft}>
        ...
        >
          {TESTIMONIALS.map((t, i) => (
            ...
            >
              <div className="font-display text-brand leading-[0.5] mb-4" style={{ fontSize: 64 }}>"</div>
              <p className="font-display text-[19px] leading-[1.5] font-medium tracking-[-0.01em] text-white">
                {t.quote}
              </p>
...
function CSCTA() {
  return (
    <section className="px-10 py-10 bg-paper">
      <motion.div
        variants={scaleIn}
        initial="hidden"
        whileInView="show"
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="mt-5 flex gap-3">
            ...
            <Link to="/expertise">
              <Btn variant="ghost" size="lg">See expertise</Btn>
            </Link>
          </div>
        </div>
      </motion.div>
