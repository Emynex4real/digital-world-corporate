...
function ExpertiseInteractive({ active, setActive }: { active: number; setActive: (i: number) => void }) {
  ...
  return (
    <section className="px-10 py-24 bg-paper-2">
      <div className="max-w-container mx-auto grid grid-cols-[1fr_1.6fr] gap-12">
        {/* Left rail */}
        <motion.div
          variants={stagger}
          initial="hidden"
        ...
        >
          {EXPERTISE.map((e, i) => (
            <motion.button
              ...
              onClick={() => setActive(i)}
              className="w-full text-left py-6 border-b border-line-2 grid grid-cols-[40px_1fr_20px] gap-4 items-center group transition-colors duration-200"
              style={{ color: active === i ? '#0a0a0a' : '#6b6b6b' }}
            >
              <span className="font-mono text-[11px]" style={{ color: active === i ? '#d97906' : '#9a9a9a' }}>
                {e.n}
              </span>
              <span className="text-[17px] tracking-[-0.01em]" style={{ fontWeight: active === i ? 600 : 500 }}>
                {e.title}
              </span>
              <span style={{ color: active === i ? '#d97906' : '#d0ccc4' }}>
                <Arrow size={14} />
              </span>
            </motion.button>
          ))}
        ...
        {/* Right panel — glassmorphism on white bg */}
        <motion.div
          key={active}
          variants={scaleIn}
          initial="hidden"
          animate="show"
          className="rounded-r-xl p-12 sticky top-[100px] shadow-lg"
          style={{ background: '#fff', border: '1px solid #e6e3dd' }}
        >
          <div className="flex justify-between items-center">
            <span className="font-mono text-[11px] tracking-[0.14em] text-brand-dark">
              Core Expertise · {a.n}
            </span>
            <span className="font-mono text-[11px] text-muted">
              0{active + 1} of 0{EXPERTISE.length}
            </span>
          </div>
          <h2
            className="mt-5 font-semibold tracking-[-0.02em] leading-[1.15]"
            style={{ fontSize: 'clamp(28px, 3vw, 40px)' }}
          >
            {a.title}
          </h2>
          <p className="mt-4 text-ink-3 text-[17px] leading-[1.65]">{a.body}</p>

          <div className="mt-8 grid grid-cols-2 gap-8">
            <div>
              <div className="font-mono text-[10px] tracking-[0.14em] uppercase text-muted mb-4">
                What's included
              </div>
            ...
            <div>
              ...
              <ul className="list-none p-0 grid gap-[10px]">
                {d.outcomes.map(x => (
                  <li key={x} className="grid grid-cols-[16px_1fr] gap-[10px] text-[14px] leading-[1.5]">
                    <span className="mt-[6px] w-1.5 h-1.5 rounded-[2px] bg-ink flex-shrink-0" />
                    {x}
                  </li>
                ))}
              </ul>
...
function ExpertiseApproach() {
  ...
  return (
    <section className="relative px-10 py-24 bg-ink text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-container mx-auto">
        ...
        >
          ...
          {steps.map(([t, b], i) => (
            <motion.div key={t} variants={fadeUp} className="relative">
              <div
                className="w-12 h-12 rounded-full border border-white/20 grid place-items-center font-display font-bold text-[18px] relative z-10 transition-colors duration-300"
                style={{ background: i === 0 ? '#f7941d' : 'rgba(255,255,255,0.06)', color: i === 0 ? '#0a0a0a' : '#fff' }}
              >
                {i + 1}
              </div>
              <div className="mt-5 text-[20px] font-semibold tracking-[-0.01em] text-white">{t}</div>
              <div className="mt-2 text-white/50 leading-[1.55] text-[14px]">{b}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
        ...
        <div className="relative z-10">
          ...
          <div className="mt-5 flex gap-3">
            ...
            <Link to="/case-studies">
              <Btn variant="ghost" size="lg">See case studies</Btn>
            </Link>
          </div>
        </div>
      </motion.div>
